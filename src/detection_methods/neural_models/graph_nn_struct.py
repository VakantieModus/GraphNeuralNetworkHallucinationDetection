...
class StructGraphNN(nn.Module):
    ...
    def set_data(
        ...
        train_dataset = TokenGraphDatasetGraphNNStruct(
            train_data, connect_sentence_clique=True, neighbor_window=window
        )
        val_dataset = (
            TokenGraphDatasetGraphNNStruct(
    ...
    def fit(
        ...
        if save:
            ...
            try:
                save_gnn(
                    self,
                    out_dir=f"GNN_MODELS/graph_nn_struct/best_{dataset}",
                    fp16=False,
...
class TokenGraphDatasetGraphNNStruct(Dataset):
    ...
    def __init__(
        ...
        include_curves: tuple[str, str, str] = (
            # "tuned_curve"#logit_curve"#, #"tuned_curve" #"curve_concat" #  ,
        ),
    ):
        super().__init__()
        self.data = data
        ...
        self.use_curves = bool(use_curves)  # <— NEW
        self.include_curves = tuple(include_curves)  # <— NEW
    ...
    def _sentence_spans_from_text_proportional(text: str, T: int):
        ...
        if not text:
            return [(0, T - 1)]
        sentences = TokenGraphDatasetGraphNNStruct._SENT_SPLIT_RE.split(text)
    ...
    def _as_token_matrix(x, T: int) -> np.ndarray:
        """
        Coerce x to float32 array of shape (T, C).
        - If x is None/empty/non-numeric → (T, 0)
        - If 1D → (len,1); tile/trim rows to T
        """
        if x is None:
    ...
    def __getitem__(self, idx):
        ...
        if self.use_curves:
            ...
            # if any curve key missing, _as_token_matrix should return shape (T,0)
            if len(mats) > 0 and sum(m.shape[1] for m in mats) > 0:
                curve_feat = np.concatenate(mats, axis=1)
            else:
...
def get_model_config(model: StructGraphNN) -> dict:
