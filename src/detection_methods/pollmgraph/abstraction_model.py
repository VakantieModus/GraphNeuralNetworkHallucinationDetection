...
from src.detection_methods.pollmgraph.utils.interfaces import Grid
...
class AbstractModel:
    ...
    def fit_transform(self, partition_model_path, train_set, val_set, test_set):
        if os.path.exists(partition_model_path):
            logging.info("Loading partition model...")
            with open(partition_model_path, "rb") as f:
                self.clustering = pickle.load(f)
            logging.info("Finished loading partition model!")
        else:
        ...
        cluster_labels_val = (
            self.clustering.predict(val_set) if len(val_set) != 0 else []
        )
...
class RegularGrid(AbstractModel):
    ...
    def fit_transform(self, partition_model_path, train_set, val_set, test_set):
        ...
        logging.info("####### Grid Partitioning #######")

        grid_num = (
            self.components
        )  # the grid number on each dimension of the reducted features
        if os.path.exists(partition_model_path):
            logging.info("Loading partition model...")
            with open(partition_model_path, "rb") as f:
                grid = pickle.load(f)
        ...
        logging.info(f"grid_num: {grid_num}")
        ...
        val_abst_traces = (
            self.pca_to_abstract_traces(grid, val_set) if len(val_set) != 0 else []
        )
