...
class TqaDataLoader(DataLoader):
    ...
    def load_hidden_states(self):
        all_instances = []
        file_path = f"{self.dataset_path}/{self.load_data()}"
        if os.path.exists(file_path):
            ...
            with open(file_path, "rb") as f:
                ...
                for data in data_list:
                    ...
                    if data.get("annotation"):
                        binary_label = 1 if data["annotation"] == "hallucinating" else 0
                        ...
                        instance_hidden_states = {
                            ...
                            "is_loop_generated": self.classify_loop(data["answer"]),
            ...
            logging.info("Generating hidden states...")
    ...
    def load_attentions(self, head_or_block):  # 0 is head, 1 is block
        ...
        file_path = f"{self.dataset_path}/{self.load_data()}"
        logging.info("Loading attentions...")
        with open(file_path, "rb") as f:
            ...
            while True:
                try:
                    ...
                    instance = {
                        "id": id,
                        "attention": (
                            data["step_by_step_attention_heads"]
                            if head_or_block == 0
...
class AdvDataLoader(DataLoader):
    ...
    def load_hidden_states(self):
        ...
        # Loop through the file list
        for file in file_list:
            with open(f"{self.dataset_path}/{file}", "rb") as f:
                pbar = tqdm(total=None, desc=f"Loading data from {file}")
                id = 0
                while True:
                    try:
                        pbar.update(1)
        ...
        logging.info("Done loading hidden states...")
        return (
            train_instances,
            val_instances,
            test_instance,
    ...
    def load_attentions(self, head_or_block):  # 0 is head, 1 is block
        ...
        for file in file_list:
            with open(f"{self.dataset_path}/{file}", "rb") as f:
                pbar = tqdm(total=None, desc=f"Loading attentions from {file}")
                id = 0
                while True:
                    try:
                        pbar.update(1)
        ...
        logging.info("Done loading attentions...")
        return (
            train_instances,
            val_instances,
            test_instance,
...
class OodDataLoader(DataLoader):
    ...
    def load_hidden_states(self):
        ...
        logging.info("Done loading hidden states...")
        logging.info(f"Number of train instances: {len(all_train_instances)}")
        train_instances, val_instances = train_test_split(
            all_train_instances, test_size=0.2, random_state=42
        )
...
class CodeLoader(DataLoader):
    def load_data(self):
        ...
        logging.info(self.llm_name, self.dataset_path)
        return file_list[0]
