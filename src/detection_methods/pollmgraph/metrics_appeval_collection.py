...
from pollmgraph.probabilistic_abstraction_model import DtmcModel, HmmModel
...
class MetricsAppEvalCollections:
    def __init__(
        ...
        result_eval_path = "{}/eval/{}".format(
            abs_args.result_save_path, abs_args.llm_name
        )
        ...
        if prob_args.model_type == "DTMC":
            ...
            self.state_positive_prob_map = (
                self.hmm_model.construct_hmmstate_state_model(train_data_points)
            )
        else:
    ...
    def get_eval_result(self):
        ...
        if self.dataset == "truthful_qa":
            ...
            if len(test_traces) < num_samples or len(val_traces) < num_samples:
                logging.info("Not enough samples to choose from!")
        ...
        logging.info(y_pred, y_groundtruth, y_prep_binary)
    ...
    def probabilistic_reasoning(self):
        ...
        The method calculates conditional probabilities for states in the abstract traces based on
        training transition probabilities.
        A divergence measure is then calculated between the matrices of probabilistic reasoning
        and training transition probabilities.

        Note:
        - The transition probabilities for training are assumed to be stored in
        `self.prob_model.train_transition_probs`.
    ...
    def value_diversity_instant_level(self):
        ...
        Note:
        - The transition probabilities are assumed to be stored in `self.prob_model.train_transition_probs`.
        - The abstract traces for testing are in `self.test_abstract_traces`, and for validation are
        in `self.val_abstract_traces`.
        - Ground truths for validation and testing are stored in `self.prob_model.val_groundtruths`
        and `self.prob_model.test_groundtruths` respectively.
    ...
    def value_diversity_n_gram_level(self):
        ...
        Note:
        - The transition probabilities are assumed to be stored in `self.prob_model.train_transition_probs`.
        - The abstract traces for testing are in `self.test_abstract_traces`, and for validation are in
        `self.val_abstract_traces`.
        - Ground truths for validation and testing are stored in `self.prob_model.val_groundtruths` and
        `self.prob_model.test_groundtruths` respectively.
        ...
        def get_result(traces, groundtruths):
            ...
            for n in n_list:
                for i, trace in enumerate(traces):
                    ...
                    (
                        result[n].append(statistics.mean(temp_n_gram_prob_list))
                        if len(temp_n_gram_prob_list) > 0
                        else 0
                    )
    ...
    def derivative_diversity_n_gram_level(self):
        ...
        Note:
        - The transition probabilities are assumed to be stored in `self.prob_model.train_transition_probs`.
        - The abstract traces for testing are in `self.test_abstract_traces`, and for
        validation are in `self.val_abstract_traces`.

        Returns:
            tuple: Four dictionaries containing the average derivative diversity for validation
            (increasing and decreasing)
                and test data (increasing and decreasing) respectively. Each dictionary has
                keys 2, 3, and 4 representing
                the n-gram level.
        """
    ...
    def _calculate_radius(self, cluster_method, data, reference_radius=None):
        logging.info(self.abstractStateExtraction.cluster_model)
    ...
    def transition_MMD_instance_level(self):
        ...
        Note:
        - The abstract traces for testing are in `self.test_abstract_traces`, and
        for validation are in `self.val_abstract_traces`.
        - The transition probabilities are assumed to be stored in `self.prob_model.train_transition_probs`.
    ...
    def transition_gain(self):
        ...
        Transition gain is computed as the difference in coverage between validation and
        test data for each state transition.
    ...
    def stationary_distribution_entropy(self):
        ...
        for key, row in TM.items():
            if sum(row.values()) == 0:
                logging.info(f"Row corresponding to key {key} has a sum of 0")
    ...
    def perplexity_llm(self):
        ...
        """
        good_probs_list = [
            p for i in self.abstractStateExtraction.train_instances for p in i["probs"]
        ]
        bad_probs_list = [
            p for i in self.abstractStateExtraction.test_instances for p in i["probs"]
        ]
        good_probs = []
        bad_probs = []
        for prob in good_probs_list:
            for p in prob:
                if p == 0:
                    p = 1e-7
                good_probs.append(p)
        for prob in bad_probs_list:
            for p in prob:
                if p == 0:
                    p = 1e-7
                bad_probs.append(p)

        logging.info(len(good_probs))
        logging.info(len(bad_probs))
    ...
    def smoothed_perplexity_abstract_model(self):
        """
        Compute the smoothed perplexity for validation and test traces based on the
        abstract model's transition probabilities.
