...
class ProbabilisticModel:
    def __init__(
        ...
        Notes:
            ...
            - The method also analyzes the length distribution of state traces in the training, validation,
            and test sets.
        """
        ...
        logging.info(f"train traces length = {len(self.train_traces)}")
        logging.info(f"val traces length = {len(self.val_traces)}")
        logging.info(f"test traces length = {len(self.test_traces)}")

        if dataset != "truthful_qa":
            logging.info("first 5 val traces: ")
            for i in range(5):
                logging.info(self.val_traces[i], self.val_instances[i]["output"])
            logging.info("first 5 test traces: ")
            for i in range(5):
                logging.info(self.test_traces[i], self.test_instances[i]["output"])

        if dataset in groundtruths_key_dataset_map:
            key = groundtruths_key_dataset_map[dataset]
        else:
            logging.info("illegal dataset")
            exit(1)
        ...
        logging.info("Analyze the length distribution...")
    ...
    def eval_llm_performance_on_dataset_task(self, task):
        ...
        Notes:
            ...
            - The method also logging.infos the attack success rate for specific tasks.
        ...
        logging.info(f"================= Accuracy of {task} ====================")
        logging.info(
        ...
        for label, positive_score_list in sentence_score_map:
            ...
            if len(positive_score_list) == 0:
                ...
                if statistic_type == "mean":
                    ...
                    logging.info("illeague statistic_type")
                    exit(1)
    ...
    def calculate_auc_probs(self, sentence_scores):
        ...
        logging.info(tpr_at_fpr_set_dist)
...
class HmmModel(ProbabilisticModel):
    def __init__(
        ...
        if dataset == "advglue++":
            if cluster_method == "Grid":
                hmm_path = "hmm_{}_components_{}_{}_{}_{}_{}_{}_{}_{}_{}.pkl".format(
                    dataset,
                    extract_block_idx,
                    hmm_components_num,
                    info_type,
                ...
                hmm_path = "hmm_{}_components_{}_{}_{}_{}_{}_{}_{}_{}.pkl".format(
                    dataset,
                    extract_block_idx,
                    hmm_components_num,
                    info_type,
    ...
    def _get_trained_hmm_models(self, data_points, n_component=150, n_iter=100):
        """
        ...
        hmm_model = hmm.GaussianHMM(
            ...
            init_params="smtc",
            random_state=42,
        ).fit(hmm_training_trace_set.reshape((-1, 1)), length_set)
    ...
    def construct_hmmstate_state_model(self, data_points):
        ...
        It determines the frequency of positive and negative labels for
        each state and calculates the state's positive probability.

        Parameters:
            data_points (list): A list of data points, where each point contains
            "step_by_step_analyzed_trace" and "label".
        ...
        logging.info(f"num_of_hmmstate = {num_of_hmmstates}")
            ...
            for j, start_hmmstate in enumerate(hmmstate_trace):
                if j + 1 < hmmstate_trace_length:
                    ...
                    if (
                        ...
                        and end_hmmstate in self.train_transition_probs[start_hmmstate]
                    ):
    ...
    def get_aucroc_by_transition_binding(self):
        ...
        if os.path.exists(self.hmm_files_path):
            logging.info(f"load hmm model from {self.hmm_files_path}")
            with open(self.hmm_files_path, "rb") as f:
                self.hmm_model = pickle.load(f)
        else:
            logging.info("start to train hmm model...")
        ...
        logging.info(len(val_test_traces), len(val_test_groundtruths))
    ...
    def get_aucroc_by_state_binding(self):
        ...
        logging.info(aucroc_score, fpr, tpr)
        return aucroc_score, fpr, tpr
...
class DtmcModel(ProbabilisticModel):
    ...
    def get_semantic_state_model(self):
        ...
        state_semantics = {
            k: round(np.mean(v) * 100) for k, v in state_semantics.items()
        }
