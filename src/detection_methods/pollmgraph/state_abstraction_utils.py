...
class AbstractStateExtraction:
    ...
    def get_cluster_label_from_pca(
        ...
        (
            ...
            cluster_labels_val,
            cluster_labels_test,
        ) = abstraction_model.fit_transform(
            partition_model_path, train_set, val_set, test_set
        )

        logging.info(f"Training set size: {len(cluster_labels_train)}")
        logging.info(f"Validation set size: {len(cluster_labels_val)}")
        logging.info(f"Test set size: {len(cluster_labels_test)}")
    ...
    def get_trace_from_hidden_states(
        ...
        logging.info("Get the training, val, and test hidden states np array...")
        ...
        logging.info("train hidden states shape", np_train_hidden_states.shape)
        if val_instances:
            logging.info("val hidden states shape", np_val_hidden_states.shape)
        logging.info("test hidden states shape", np_test_hidden_states.shape)

        # ========= 3. PCA FIT and Transform =========
        logging.info(f"PCA dimension: {pca_dim}, Starting PCA...")

        if os.path.exists(pca_model_path):
            logging.info("Loading pca model and partition model...")
            with open(pca_model_path, "rb") as f:
                pca = pickle.load(f)

            logging.info("Finished loading pca model!")
        else:
        ...
        logging.info("PCA fitting finished!")
        ...
        logging.info("PCA transform finished!")
        ...
        logging.info("Format the traces based on sentences...")
    ...
    def get_trace_from_attention(
        ...
        Notes:
            ...
            - Clustering is performed on the reduced data to obtain state traces.
        """
        logging.info("Get the training, val, and test attention np array...")
        ...
        logging.info("train attention shape", np_train_attention.shape)
        logging.info("val attention shape", np_val_attention.shape)
        logging.info("test attention shape", np_test_attention.shape)

        # ========= 3. PCA FIT and Transform =========
        logging.info(f"PCA dimension: {pca_dim}")
    ...
    def main(self, train_instances, val_instances, test_instances):
        ...
        This method reads configurations from the 'args' attribute, determines the
        type of information (hidden states or attention)
        ...
        pca_model_folder_path = f"{dataset_folder_path}/pca_model"
        ...
        partition_model_folder_path = f"{dataset_folder_path}/partition_model"
        ...
        if train_instances and val_instances and test_instances:
            ...
            if info_type == "hidden_states":
                logging.info("Loading hidden states...")
                (
                    train_instances,
                    val_instances,
                    test_instances,
                ) = loader.load_hidden_states()
                logging.info("Finished loading hidden states!")
                ...
                if info_type == "attention_heads":
                    logging.info("Loading attention heads...")
                    (
                        train_instances,
                        val_instances,
                        test_instances,
                    ) = loader.load_attentions(0)
                    logging.info("Finished loading attention heads!")
                else:
                    logging.info("Loading attention blocks...")
                    (
                        train_instances,
                        val_instances,
                        test_instances,
                    ) = loader.load_attentions(1)
                    logging.info("Finished loading attention blocks!")
        ...
        logging.info("Finished training and test traces generation!")
