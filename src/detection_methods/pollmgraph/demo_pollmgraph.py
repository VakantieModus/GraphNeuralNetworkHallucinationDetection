...
import torch
from pollmgraph.abstraction_model import GMM, KMeans
...
# # Function to logging.info with color
# def logging.info_color(text, color):
#     logging.info(color + text + Colors.ENDC)
...
def load_abstract_model(pca_model, partition_model, train_instances):
    logging.info_color("Loading abstract model...", Colors.HEADER)

    if os.path.exists(
        "eval/demo/{}_{}_{}_{}_{}_dtmc_model.pkl".format(
            llm, dataset, extract_block_idx, pca_dim, abstraction_method
        )
    ):
        with open(
            "eval/demo/{}_{}_{}_{}_{}_dtmc_model.pkl".format(
        ...
        state_positive_prob_map = construct_dtmc_state_model(train_data_points)
...
def get_demo_input():
    logging.info_color("Please type in the input: ", Colors.OKCYAN)
    input_text = input()  # Replace with actual input retrieval if necessary
    return input_text


def perform_inference(input_text):
    logging.info_color("Performing inference...", Colors.WARNING)
    ...
    def parse_hidden_states(hidden_states, layer_indices=None):
        ...
        logging.info(
            len(hidden_states), len(hidden_states[0]), hidden_states[0][0].shape
        )
    ...
    def truthful_qa_inference(
        ...
        question_text = (
            "You are a helpful, respectful and honest assistant with a deep knowledge of "
            "code and software design. Always answer as helpfully as possible, while "
            "being safe. Your answers should not include any harmful, unethical, racist, "
    ...
    # if llm == "llama2_7B" or llm == "code_llama_7B_python":
...
def send_output_to_abstract_model(output, abstract_model, pca_model, partition_model):
    logging.info_color("Sending output to LUNA...", Colors.OKBLUE)
...
def show_generated_result(abstract_trace, probabilities, final_score):
    logging.info_color("Generated result:", Colors.BOLD)
    logging.info_color(f"Abstract Trace: {abstract_trace}", Colors.OKGREEN)
    logging.info_color(f"Probabilities: {probabilities}", Colors.OKGREEN)
    logging.info_color(f"Final Score: {final_score}", Colors.OKGREEN)
...
def main():
    ...
    abstract_trace, probabilities, score = send_output_to_abstract_model(
        output["hidden_states"], dtmc_model, pca_model, partition_model
    )
