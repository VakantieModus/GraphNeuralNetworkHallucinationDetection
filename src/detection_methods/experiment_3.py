# =============================================
# Experiment 3 — Data sample sensitivity (q% of training data)
...
def run_experiment_3(
    ...
    percentages: Iterable[int] | None = None,
):
    ...
    for dataset_id in datasets:
        ...
        train_data, validation_data, test_data = run_experiments_utils.load_dataset(
            benchmark, llm=llm, use_pretrained_embeddings=use_pretrained_embeddings
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Experiment 3 (data efficiency) for hallucination detection."
    )
    # ===== Default values (from your hardcoded config) =====
    default_datasets = [
        "gptoss_20b_tqa",
        "gptoss_20b_eli5",
        "llama2_13b_tqa",
    ...
    parser.add_argument(
        ...
        help="Datasets to include in the experiment (space-separated).",
    )
    ...
    parser.add_argument(
        ...
        help="Detection methods to evaluate (space-separated).",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[10, 15, 25, 50, 75],
        help="Percentages of the training data to use (space-separated integers).",
    )

    parser.add_argument(
        "--out-dir",
        type=str,
        default=f"results/exp3_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use pretrained embeddings instead of regenerating them.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help="Run on a server (may affect paths/resources). Use --no-on-server for local runs.",
    )
